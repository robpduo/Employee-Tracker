...
const { viewAllEmployees, updateEmployeeRole, addNewEmployee } = require('./Employee');
const { viewAllDepartments, addNewDepartment } = require('./Departments');
const { viewAllRoles, addNewRole } = require('./Role');
const { viewUtilizedBudget } = require('./Budget');
...
class Prompter {
    ...
    promptMenu() {
        return inquirer.prompt({
            ...
            choices: ['View All Employees', 'Update Employee Role', 'Add Employee', 'View All Roles', 'Add Role', 'View All Departments', 'Add Department', 'Total Utilized Budget','Quit']
        }).then(async response => {
            if (response.menuSelection === 'View All Employees') {
            ...
            } else if (response.menuSelection === 'Add Employee') {
                await this.inquireNewEmployee();

                console.log('');
                console.log("Who is this employees manager?");
                await this.inquireEmployeeName();


                console.log('');
                console.log('What is this employees role?');
                await this.inquireRole();
                ...
                await addNewEmployee(employeeObj);
                console.log('');
            ...
            } else if (response.menuSelection === 'Total Utilized Budget') {
                const employeeSalary = await viewUtilizedBudget();
                console.table(employeeSalary.data);

                let budget = 0;
                employeeSalary.data.map(salary => { budget += parseFloat(salary.salary)});
